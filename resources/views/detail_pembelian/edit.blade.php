...
<h1 class="m-0 text-light">Edit Detail Pembelian</h1>
@stop
@section('content')
...
placeholder="Id Pembelian" id="nonota_beli" name="nonota_beli"
value="{{ $detail_pembelian->fpembelian->nonota_beli ?? old('nonota_beli') }}"
aria-label="Id Pembelian" ariadescribedby="cari" readonly>
...
id="tgl_kadaluarsa" placeholder="Tanggal Kadaluarsa" name="tgl_kadaluarsa"
value="{{ $detail_pembelian->tgl_kadaluarsa ?? old('tgl_kadaluarsa') }}">
...
<input type="text" class="form-control @error('subtotal_beli') is-invalid @enderror"
id="subtotal_beli" placeholder="Subtotal Beli" name="subtotal_beli"
value="{{ $detail_pembelian->subtotal_beli ?? old('subtotal_beli') }}">
@error('subtotal_beli')
<span class="text-danger">{{ $message }}</span>
@enderror
</div>
...
<input type="text" class="form-control @error('persen_margin_jual') is-invalid @enderror"
...
value="{{ $detail_pembelian->fobat->nama_obat ?? old('nama_obat') }}"
...
<td>
<button type="button" class="btn btn-primary btn-xs" onclick="pilih1('{{$pembelian->id}}', '{{$pembelian->nonota_beli}}')" data-bs-dismiss="modal">Pilih</button>
</td>
</tr>
@endforeach
</tbody>
